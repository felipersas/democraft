...
export const locatorSchema: z.ZodType<Locator> = z.discriminatedUnion("kind", [
  z
    ...
    .passthrough(),
  z.object({ kind: z.literal("label"), text: z.string() }).passthrough(),
  z.object({ kind: z.literal("testId"), id: z.string() }).passthrough(),
  z.object({ kind: z.literal("text"), text: z.string() }).passthrough(),
]);

export const targetDefinitionSchema: z.ZodType<TargetDefinition> = z
  .object({
    id: z.string().min(1),
