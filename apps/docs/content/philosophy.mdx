...
The authoring surface — `defineDemo`, `defineTargets`, the `DemoScene` builder, the fourteen step kinds — is identical whether a human writes a demo by hand or an AI agent generates one. There is no separate "agent SDK" or JSON-only format.
