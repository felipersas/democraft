...
Capture failures should include the `trace.zip` from `.democraft/runs/<id>/` — it lets others reproduce the exact Playwright session. Run `inspect --static` and `validate --static` first to rule out structural demo errors.
